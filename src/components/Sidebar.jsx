...
function Sidebar() {
  ...
  const userName =
    localStorage.getItem("usuarioLogado?.nome") || "Administrador";
  ...
  const btnClass =
    "w-full text-left px-4 py-3 rounded-xl transition-all duration-200 text-slate-300 hover:bg-slate-700 hover:text-blue-400 font-medium flex items-center gap-3";
  ...
  return (
    <div className="w-64 h-screen bg-slate-800 border-r fixed left-0 top-0 border-slate-700 p-4 flex flex-col shadow-2xl">
      ...
      <nav className="flex-1 space-y-2">
        ...
        <button onClick={() => navigate("/lancamentos")} className={btnClass}>
